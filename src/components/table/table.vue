<template>
  <div class="cl-table-wrapper" ref="wrapper">
    <div :style="{height, overflow: 'auto'}" ref="tableWrapper">
      <table ref="table" class="cl-table" :class="{bordered, compact, striped: striped}">
        <thead>
        <tr>
          <th v-if="expendField" :style="{width: '50px'}" class="cl-table-center"></th>
          <th v-if="checkable" :style="{width: '50px'}" class="cl-table-center">
          ...
          <th :style="{width: column.width + 'px'}" v-for="column in columns" :key="column.field">
            <div class="cl-table-header">
              {{column.text}}
              <span v-if="column.field in orderBy" class="cl-table-sorter" @click="changeOrderBy(column.field)">
                <cl-icon name="asc" :class="{active: orderBy[column.field] === 'asc'}"/>
                <cl-icon name="desc" :class="{active: orderBy[column.field] === 'desc'}"/>
              </span>
            </div>
          </th>
        ...
        <tbody ref="tbody">
          <template v-for="(item,index) in dataSource">
            <tr :key="item.id">
              <td v-if="expendField" :style="{width: '50px'}" class="cl-table-center">
                <cl-icon class="cl-table-expendIcon" name="right" :class="{expended: expendedIds.indexOf(item.id) >= 0}"
                  @click="expendItem(item.id)"/>
              </td>
              <td v-if="checkable" :style="{width: '50px'}" class="cl-table-center">
    ...
    <div class="cl-table-loading" v-if="loading">
      <cl-icon name="loading" />
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.table2 = table2
      table2.classList.add('cl-table-copy')
