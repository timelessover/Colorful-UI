<template>
  <div ref="parent" class="cl-scroll-wrapper" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" @cl="oncl"
    @mousemove="onMouseMove">
    <div ref="child" class="cl-scroll" :style="{transform: `translateY(${this.contentY}px)`}">
      <slot></slot>
    </div>
    <div class="cl-scroll-track" v-show="scrollBarVisible">
      <div class="cl-scroll-bar" ref="bar" @mousedown="onMouseDownScrollBar" @selectstart="onSelectStartScrollBar">
        <div class="cl-scroll-bar-inner"></div>
      </div>
    </div>
  </div>
</template>
    ...
    methods: {
      listenToRemoteResources () {
        ...
        Array.from(tags).map((tag) => {
          if (tag.hasAttribute('data-cl-listened')) { return }
          tag.setAttribute('data-cl-listened', 'yes')
